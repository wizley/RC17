...
/*===========================================================================*/
USBHFTDIDriver USBHFTDID[DRIVER_USBHFTDI_MAX_INSTANCES];
...
static USBHFTDIPortDriver *_find_port(void) {
	uint8_t i;
	for (i = 0; i < DRIVER_USBHFTDI_MAX_PORTS; i++) {
...
static usbh_baseclassdriver_t *_ftdi_load(usbh_device_t *dev, const uint8_t *descriptor, uint16_t rem) {
	...
	/* alloc driver */
	for (i = 0; i < DRIVER_USBHFTDI_MAX_INSTANCES; i++) {
...
USBHFTDIPortDriver FTDIPD[DRIVER_USBHFTDI_MAX_PORTS];
...
void usbhftdipStart(USBHFTDIPortDriver *ftdipp, const USBHFTDIPortConfig *config) {
	static const USBHFTDIPortConfig default_config = {
		DRIVER_USBHFTDI_DEFAULT_SPEED,
		DRIVER_USBHFTDI_DEFAULT_FRAMING,
		DRIVER_USBHFTDI_DEFAULT_HANDSHAKE,
		DRIVER_USBHFTDI_DEFAULT_XON,
		DRIVER_USBHFTDI_DEFAULT_XOFF
	};
