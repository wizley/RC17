...
USBHMassStorageDriver USBHMSD[DRIVER_USBHMSD_MAX_INSTANCES];
...
static usbh_baseclassdriver_t *_msd_load(usbh_device_t *dev, const uint8_t *descriptor, uint16_t rem) {
	...
	/* alloc driver */
	for (i = 0; i < DRIVER_USBHMSD_MAX_INSTANCES; i++) {
...
alloc_ok:
	...
	if (stat == USBH_URBSTATUS_OK) {
		...
		if (msdp->max_lun > DRIVER_USBHMSD_MAX_LUNS) {
			msdp->max_lun = DRIVER_USBHMSD_MAX_LUNS;
	...
	luns = msdp->max_lun;
	for (i = 0; (luns > 0) && (i < DRIVER_USBHMSD_MAX_LUNS); i++) {
...
USBHMassStorageLUNDriver MSBLKD[DRIVER_USBHMSD_MAX_LUNS];
