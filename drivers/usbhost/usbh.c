...
#if DRIVER_USBH_USE_HUB
	uint8_t i;
	for (i = 0; i < DRIVER_USBHHUB_MAX_INSTANCES; i++) {
...
static bool _device_enumerate(usbh_device_t *dev) {
	...
	osalThreadSleepMilliseconds(DRIVER_USBH_DEVICE_ADDRESS_STABILIZATION);
...
static void _port_connected(usbh_port_t *port) {
	...
	osalThreadSleepMilliseconds(DRIVER_USBH_PORT_DEBOUNCE_TIME);
...
reset:
	for (i = 0; i < 3; i++) {
		...
		while (TRUE) {
			...
			/* check for timeout */
			if (osalOsGetSystemTimeX() - start > DRIVER_USBH_PORT_RESET_TIMEOUT) break;
		}
	}
