...
#if DRIVER_USE_LTDC || defined(__DOXYGEN__)
...
CH_IRQ_HANDLER(LTDC_EV_IRQHandler) {
  ...
  if ((LTDC->ISR & LTDC_ISR_LIF) && (LTDC->IER & LTDC_IER_LIE)) {
    chDbgAssert(ltdcp->config->line_isr != NULL, "invalid state");
  ...
  if ((LTDC->ISR & LTDC_ISR_RRIF) && (LTDC->IER & LTDC_IER_RRIE)) {
    ...
    chSysLockFromISR();
    chDbgAssert(ltdcp->state == LTDC_ACTIVE, "invalid state");
#if LTDC_USE_WAIT
    /* Wake the waiting thread up.*/
    if (ltdcp->thread != NULL) {
      thread_t *tp = ltdcp->thread;
      ltdcp->thread = NULL;
...
CH_IRQ_HANDLER(LTDC_ER_IRQHandler) {
  ...
  if ((LTDC->ISR & LTDC_ISR_FUIF) && (LTDC->IER & LTDC_IER_FUIE)) {
    chDbgAssert(ltdcp->config->fuerr_isr != NULL, "invalid state");
  ...
  if ((LTDC->ISR & LTDC_ISR_TERRIF) && (LTDC->IER & LTDC_IER_TERRIE)) {
    chDbgAssert(ltdcp->config->terr_isr != NULL, "invalid state");
...
void ltdcStart(LTDCDriver *ltdcp, const LTDCConfig *configp) {
  ...
  chDbgCheck(ltdcp == &LTDCD1);
  chDbgCheck(configp != NULL);
  chDbgAssert(ltdcp->state == LTDC_STOP, "invalid state");
  ...
  chDbgAssert(configp->hsync_width >= LTDC_MIN_HSYNC_WIDTH, "outside range");
  chDbgAssert(configp->hsync_width <= LTDC_MAX_HSYNC_WIDTH, "outside range");
  chDbgAssert(configp->vsync_height >= LTDC_MIN_VSYNC_HEIGHT, "outside range");
  chDbgAssert(configp->vsync_height <= LTDC_MAX_VSYNC_HEIGHT, "outside range");
  ...
  chDbgAssert(configp->hbp_width >= LTDC_MIN_HBP_WIDTH, "outside range");
  chDbgAssert(configp->hbp_width <= LTDC_MAX_HBP_WIDTH, "outside range");
  chDbgAssert(configp->vbp_height >= LTDC_MIN_VBP_HEIGHT, "outside range");
  chDbgAssert(configp->vbp_height <= LTDC_MAX_VBP_HEIGHT, "outside range");

  hacc += configp->hbp_width;
  vacc += configp->vbp_height;

  chDbgAssert(hacc + 1 >= LTDC_MIN_ACC_HBP_WIDTH, "outside range");
  chDbgAssert(hacc + 1 <= LTDC_MAX_ACC_HBP_WIDTH, "outside range");
  chDbgAssert(vacc + 1 >= LTDC_MIN_ACC_VBP_HEIGHT, "outside range");
  chDbgAssert(vacc + 1 <= LTDC_MAX_ACC_VBP_HEIGHT, "outside range");
  ...
  chDbgAssert(configp->screen_width >= LTDC_MIN_SCREEN_WIDTH, "outside range");
  chDbgAssert(configp->screen_width <= LTDC_MAX_SCREEN_WIDTH, "outside range");
  chDbgAssert(configp->screen_height >= LTDC_MIN_SCREEN_HEIGHT, "outside range");
  chDbgAssert(configp->screen_height <= LTDC_MAX_SCREEN_HEIGHT, "outside range");

  hacc += configp->screen_width;
  vacc += configp->screen_height;

  chDbgAssert(hacc + 1 >= LTDC_MIN_ACC_ACTIVE_WIDTH, "outside range");
  chDbgAssert(hacc + 1 <= LTDC_MAX_ACC_ACTIVE_WIDTH, "outside range");
  chDbgAssert(vacc + 1 >= LTDC_MIN_ACC_ACTIVE_HEIGHT, "outside range");
  chDbgAssert(vacc + 1 <= LTDC_MAX_ACC_ACTIVE_HEIGHT, "outside range");
  ...
  chDbgAssert(configp->hfp_width >= LTDC_MIN_HFP_WIDTH, "outside range");
  chDbgAssert(configp->hfp_width <= LTDC_MAX_HFP_WIDTH, "outside range");
  chDbgAssert(configp->vfp_height >= LTDC_MIN_VFP_HEIGHT, "outside range");
  chDbgAssert(configp->vfp_height <= LTDC_MAX_VFP_HEIGHT, "outside range");

  hacc += configp->hfp_width;
  vacc += configp->vfp_height;

  chDbgAssert(hacc + 1 >= LTDC_MIN_ACC_TOTAL_WIDTH, "outside range");
  chDbgAssert(hacc + 1 <= LTDC_MAX_ACC_TOTAL_WIDTH, "outside range");
  chDbgAssert(vacc + 1 >= LTDC_MIN_ACC_TOTAL_HEIGHT, "outside range");
  chDbgAssert(vacc + 1 <= LTDC_MAX_ACC_TOTAL_HEIGHT, "outside range");
...
//                   CORTEX_PRIO_MASK(STM32_LTDC_EV_IRQ_PRIORITY));
//  nvicEnableVector(STM32_LTDC_ER_NUMBER,
//                   CORTEX_PRIO_MASK(STM32_LTDC_ER_IRQ_PRIORITY));
  nvicEnableVector(STM32_LTDC_EV_NUMBER,
                   STM32_LTDC_EV_IRQ_PRIORITY);
  nvicEnableVector(STM32_LTDC_ER_NUMBER,
                   STM32_LTDC_ER_IRQ_PRIORITY);
...
void ltdcStop(LTDCDriver *ltdcp) {
  ...
  chSysLock();
  chDbgAssert(ltdcp->state == LTDC_READY, "invalid state");
...
void ltdcStartReloadI(LTDCDriver *ltdcp, bool immediately) {
  ...
  chDbgCheckClassI();
  chDbgCheck(ltdcp == &LTDCD1);
  chDbgAssert(ltdcp->state == LTDC_READY, "not ready");
  (void)ltdcp;
...
#if LTDC_USE_WAIT
  chDbgAssert(ltdcp->thread == NULL, "already waiting");
...
void ltdcBgSetPaletteColorI(LTDCDriver *ltdcp, uint8_t slot, ltdc_color_t c) {
  ...
  chDbgCheckClassI();
  chDbgCheck(ltdcp == &LTDCD1);
  chDbgAssert(!ltdcBgIsEnabledI(ltdcp), "invalid state");
  (void)ltdcp;
...
void ltdcBgSetPaletteI(LTDCDriver *ltdcp, const ltdc_color_t colors[],
  ...
  chDbgCheckClassI();
  chDbgCheck(ltdcp == &LTDCD1);
  chDbgCheck((colors == NULL) == (length == 0));
  chDbgAssert(length <= LTDC_MAX_PALETTE_LENGTH, "outside range");
  chDbgAssert(!ltdcBgIsEnabledI(ltdcp), "invalid state");
  (void)ltdcp;
...
void ltdcBgSetPixelFormatI(LTDCDriver *ltdcp, ltdc_pixfmt_t fmt) {
  ...
  chDbgCheckClassI();
  chDbgCheck(ltdcp == &LTDCD1);
  chDbgAssert(fmt >= LTDC_MIN_PIXFMT_ID, "outside range");
  chDbgAssert(fmt <= LTDC_MAX_PIXFMT_ID, "outside range");
  (void)ltdcp;
...
void ltdcBgSetWindowI(LTDCDriver *ltdcp, const ltdc_window_t *windowp) {
  ...
  chDbgAssert(windowp->hstop < ltdcp->config->screen_width, "outside range");
  chDbgAssert(windowp->vstop < ltdcp->config->screen_height, "outside range");

  /* Horizontal boundaries.*/
  start = (uint32_t)windowp->hstart + ltdcp->active_window.hstart;
  stop  = (uint32_t)windowp->hstop  + ltdcp->active_window.hstart;

  chDbgAssert(start >= ltdcp->active_window.hstart, "outside range");
  chDbgAssert(stop <= ltdcp->active_window.hstop, "outside range");
  ...
  chDbgAssert(start >= ltdcp->active_window.vstart, "outside range");
  chDbgAssert(stop <= ltdcp->active_window.vstop, "outside range");
...
void ltdcBgSetFrameI(LTDCDriver *ltdcp, const ltdc_frame_t *framep) {
  ...
  chDbgAssert(framep->width  <= ltdcp->config->screen_width, "outside range");
  chDbgAssert(framep->height <= ltdcp->config->screen_height, "outside range");
  chDbgAssert(linesize >= LTDC_MIN_FRAME_WIDTH_BYTES, "outside range");
  chDbgAssert(linesize <= LTDC_MAX_FRAME_WIDTH_BYTES, "outside range");
  chDbgAssert(framep->height >= LTDC_MIN_FRAME_HEIGHT_LINES, "outside range");
  chDbgAssert(framep->height <= LTDC_MAX_FRAME_HEIGHT_LINES, "outside range");
  chDbgAssert(framep->pitch  >= linesize, "outside range");
...
void ltdcFgSetPaletteColorI(LTDCDriver *ltdcp, uint8_t slot, ltdc_color_t c) {
  ...
  chDbgCheckClassI();
  chDbgCheck(ltdcp == &LTDCD1);
  chDbgAssert(!ltdcFgIsEnabledI(ltdcp), "invalid state");
  (void)ltdcp;
...
size_t ltdcBitsPerPixel(ltdc_pixfmt_t fmt) {
  ...
  chDbgAssert(fmt < LTDC_MAX_PIXFMT_ID, "invalid format");
...
ltdc_color_t ltdcToARGB8888(ltdc_color_t c, ltdc_pixfmt_t fmt) {
  ...
  default:
    chDbgAssert(FALSE, "invalid format");
    return 0;
  }
}
