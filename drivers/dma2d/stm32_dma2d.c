...
#if DRIVER_USE_DMA2D || defined(__DOXYGEN__)
...
CH_IRQ_HANDLER(DMA2D_IRQHandler) {
  ...
  if (job_done) {
    chSysLockFromISR();
    chDbgAssert(dma2dp->state == DMA2D_ACTIVE, "invalid state");

  #if DMA2D_USE_WAIT
    /* Wake the waiting thread up.*/
    if (dma2dp->thread != NULL) {
      thread_t *tp = dma2dp->thread;
      dma2dp->thread = NULL;
...
void dma2dStart(DMA2DDriver *dma2dp, const DMA2DConfig *configp) {
  ...
  chDbgAssert(dma2dp->state == DMA2D_STOP, "invalid state");
...
//		  	  	  CORTEX_PRIO_MASK(STM32_DMA2D_IRQ_PRIORITY));
  nvicEnableVector(STM32_DMA2D_NUMBER,
		  	  	  STM32_DMA2D_IRQ_PRIORITY);
...
void dma2dStop(DMA2DDriver *dma2dp) {
  ...
  chDbgCheck(dma2dp == &DMA2DD1);
  chDbgAssert(dma2dp->state == DMA2D_READY, "invalid state");
#if DMA2D_USE_WAIT
  chDbgAssert(dma2dp->thread == NULL, "still waiting");
#endif /* DMA2D_USE_WAIT */
...
void dma2dAcquireBusS(DMA2DDriver *dma2dp) {
  ...
  chDbgCheckClassS();
  chDbgCheck(dma2dp == &DMA2DD1);
  chDbgAssert(dma2dp->state == DMA2D_READY, "not ready");
...
void dma2dSetDeadTimeI(DMA2DDriver *dma2dp, uint32_t cycles) {
  ...
  chDbgCheckClassI();
  chDbgCheck(dma2dp == &DMA2DD1);
  chDbgAssert(cycles <= DMA2D_MAX_DEADTIME_CYCLES, "outside range");
  (void)dma2dp;
...
void dma2dJobSetModeI(DMA2DDriver *dma2dp, dma2d_jobmode_t mode) {
  ...
  chDbgCheck(dma2dp == &DMA2DD1);
  chDbgAssert(dma2dp->state == DMA2D_READY, "not ready");
  chDbgAssert((mode & ~DMA2D_CR_MODE) == 0, "outside range");
  (void)dma2dp;
...
void dma2dJobGetSizeI(DMA2DDriver *dma2dp,
  ...
  chDbgCheckClassI();
  chDbgCheck(dma2dp == &DMA2DD1);
  chDbgCheck(widthp != NULL);
  chDbgCheck(heightp != NULL);
  (void)dma2dp;
...
void dma2dJobSetSizeI(DMA2DDriver *dma2dp, uint16_t width, uint16_t height) {
  ...
  chDbgCheck(dma2dp == &DMA2DD1);
  chDbgAssert(dma2dp->state == DMA2D_READY, "not ready");
  chDbgAssert(width <= DMA2D_MAX_WIDTH, "outside range");
  chDbgAssert(height <= DMA2D_MAX_HEIGHT, "outside range");
  (void)dma2dp;
...
void dma2dJobSuspendI(DMA2DDriver *dma2dp) {
  ...
  chDbgCheckClassI();
  chDbgCheck(dma2dp == &DMA2DD1);
  chDbgCheck((DMA2D->CR & DMA2D_CR_SUSP) == 0);
  chDbgAssert(dma2dp->state == DMA2D_ACTIVE, "invalid state");
...
void dma2dJobResumeI(DMA2DDriver *dma2dp) {
  ...
  chDbgCheck(dma2dp == &DMA2DD1);
  chDbgCheck((DMA2D->CR & DMA2D_CR_SUSP) != 0);
  chDbgAssert(dma2dp->state == DMA2D_PAUSED, "invalid state");
...
void dma2dBgSetWrapOffsetI(DMA2DDriver *dma2dp, size_t offset) {
  ...
  chDbgCheck(dma2dp == &DMA2DD1);
  chDbgAssert(dma2dp->state == DMA2D_READY, "not ready");
  chDbgAssert(offset <= DMA2D_MAX_OFFSET, "outside range");
  (void)dma2dp;
...
void dma2dBgSetAlphaModeI(DMA2DDriver *dma2dp, dma2d_amode_t mode) {
  ...
  chDbgCheck(dma2dp == &DMA2DD1);
  chDbgAssert(dma2dp->state == DMA2D_READY, "not ready");
  chDbgAssert((mode & ~DMA2D_BGPFCCR_AM) == 0, "outside range");
  chDbgAssert((mode & DMA2D_BGPFCCR_AM) != DMA2D_BGPFCCR_AM, "outside range");
  (void)dma2dp;
...
void dma2dBgSetPixelFormatI(DMA2DDriver *dma2dp, dma2d_pixfmt_t fmt) {
  ...
  chDbgCheck(dma2dp == &DMA2DD1);
  chDbgAssert(dma2dp->state == DMA2D_READY, "not ready");
  chDbgAssert(fmt <= DMA2D_MAX_PIXFMT_ID, "outside range");
  (void)dma2dp;
...
void dma2dBgSetPaletteS(DMA2DDriver *dma2dp, const dma2d_palcfg_t *palettep) {
  ...
  chDbgAssert(dma2dp->state == DMA2D_READY, "not ready");
  chDbgCheck(palettep != NULL);
  chDbgCheck(palettep->colorsp != NULL);
  chDbgAssert(palettep->length > 0, "outside range");
  chDbgAssert(palettep->length <= DMA2D_MAX_PALETTE_LENGTH, "outside range");
...
void dma2dFgSetAlphaModeI(DMA2DDriver *dma2dp, dma2d_amode_t mode) {
  ...
  chDbgCheck(dma2dp == &DMA2DD1);
  chDbgAssert(dma2dp->state == DMA2D_READY, "not ready");
  chDbgAssert((mode & ~DMA2D_FGPFCCR_AM) == 0, "outside range");
  chDbgAssert((mode & DMA2D_FGPFCCR_AM) != DMA2D_FGPFCCR_AM, "outside range");
  (void)dma2dp;
...
void dma2dOutSetPixelFormatI(DMA2DDriver *dma2dp, dma2d_pixfmt_t fmt) {
  ...
  chDbgCheck(dma2dp == &DMA2DD1);
  chDbgAssert(dma2dp->state == DMA2D_READY, "not ready");
  chDbgAssert(fmt <= DMA2D_MAX_OUTPIXFMT_ID, "outside range");
  (void)dma2dp;
...
size_t dma2dBitsPerPixel(dma2d_pixfmt_t fmt) {
  ...
  chDbgAssert(fmt < DMA2D_MAX_PIXFMT_ID, "invalid format");
